...
impl Plugin for GifMakerPlugin {
    fn build(&self, app: &mut App) {
        ...
        if !scratch_dir.exists() && let Err(e) = std::fs::create_dir_all(&scratch_dir) {
        ...
        app.add_systems(FixedUpdate, (continuous_capture.run_if(resource_exists_and_equals(Shooting(true))),));
    }
}
