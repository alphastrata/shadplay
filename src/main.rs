...
fn run_normal(shader_arg: Option<String>) {
    if let Some(ref path) = shader_arg
        && let Err(e) = utils::apply_shader_file(Path::new(path)) {
...
fn run_screensaver(dir: &str, shader_arg: Option<String>) {
    ...
    let mut shaders: Vec<std::path::PathBuf> = fs::read_dir(dir_path).into_iter()
    ...
    app.insert_resource(ClearColor(Color::BLACK))
        .insert_resource(ScreensaverState { shaders, index: 1 % shader_count, interval: 15.0 })
        .add_plugins((
            DefaultPlugins.set(WindowPlugin {
