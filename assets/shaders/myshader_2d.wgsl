...
fn blackhole(uv: vec2f, resolution: vec2f, t: f32, tex: vec4f) -> vec4f {
    var colOut = vec4(0.0);

    var frag_rotation = vec2f(0.0);
    frag_rotation.x = uv.x * 0.985 + uv.y * 0.174;
    frag_rotation.y = uv.y * 0.985 - uv.x * 0.174;
    frag_rotation += vec2(-0.06, 0.12) * resolution.xy;

    for (var j: i32 = 0; j < ANTI_ALIASING; j++) {
        for (var i: i32 = 0; i < ANTI_ALIASING; i++) {
            ...
            if outCol.r == 100.0 {
                ...
                colOut = vec4f(col_rgb, col.a) / (f32(ANTI_ALIASING) * f32(ANTI_ALIASING));
            }
        }
