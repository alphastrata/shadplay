...
fn fragment(in: MeshVertexOutput) -> @location(0) vec4<f32> {
    ...
    let off6: array<vec2<f32>, 6> = array<vec2<f32>, 6>(
        vec2<f32>(off6(0.)),
        vec2<f32>(off6(1.)),
        vec2<f32>(off6(2.)),
        vec2<f32>(off6(3.)),
    ...
    // Apply distortion if defined
    // #ifdef DISTORT
    //     let nhp: vec2<f32> = transform(hp + aa);
    //     hp = transform(hp);
    //     let scale: f32 = distance(nhp, hp);
    // #else
    hp *= 3.0;
    // #endif
    ...
    // Define the maximum distance for distortion
    // #ifdef DISTORT
    //     let mx: f32 = 0.0005 + 0.01 * scale;
    // #else
    let mx: f32 = 0.0005;
    // #endif
    ...
    for (var i = 0; i <= 6; i ++) {
        ...
        // // Apply flicker effect if defined
        // // #if defined(FLICKER)
        // //     let fade: f32 = (smoothstep(1.05, 0.9, dot(p0 - p1, p0 - p1)) > hash(floor(TIME * 30.0) + h1)) ? 1.0 : 0.0;
        // // #else
    ...
    // // Add scale-based color contribution if distortion is defined
    // // #ifdef DISTORT
    // //     col += scale * bcol;
    // // #endif
